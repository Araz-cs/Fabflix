...
//        String id = request.getParameter("id");
        ...
        try (Connection conn = dataSource.getConnection()) {
            ...
            String query = "SELECT m.movieID as mid, m.title as title, m.yearz as year, m.director as director,substring_index(group_concat(distinct g.gNames SEPARATOR ', '), ', ', 3) as genres,substring_index(group_concat(s.id SEPARATOR ','), ',', 3) as sid,substring_index(group_concat(s.name SEPARATOR ', '), ', ', 3) as stars ,r.rating\n" +
                    "                    FROM movies m\n" +
            ...
            while (rs.next()) {
                ...
                ArrayList<String> arrSN = new ArrayList<>(Arrays.asList(sname.split(",")));
                try {
                    jsonObject.addProperty("0n", arrSN.get(0));
                    jsonObject.addProperty("1n", arrSN.get(1));
                    jsonObject.addProperty("2n", arrSN.get(2));
                }
                catch (Exception ignored)
                {
